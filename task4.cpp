...
void sumDiag(matrix matx)
{
    int primSum = 0, secSum = 0; // values to keep track of diagonal sums
    for (int i = 0; i < matx.matx.size(); i++)
    {                                                           // loops across matrix
        primSum += stoi(matx.matx[i][i]);                       // adds primary diagonal of index i,i
        secSum += stoi(matx.matx[i][matx.matx.size() - i - 1]); // adds secondary diagonal of index i, n-1-i
    }

    cout << "Primary Diagonal Sum: ";   // prompt for primary diagonal
    cout << to_string(primSum) << endl; // prints primary diagonal
    cout << "Secondary Diagonal Sum: "; // prompt for secondary diagonal
    cout << to_string(secSum) << endl;  // prints secondary diagonal
...
int main()
{
    ...
    cout << "Matrix 1 diagonals" << endl; // prompt for matrix 1
    sumDiag(matx1);                       // prints matrix 1 diagonals

    cout << "Matrix 2 diagonals" << endl; // prompt for matrix 2
    sumDiag(matx2);                       // prints matrix 2 diagonals
